...
def csvsort(
    ...
    reader = _get_reader(
        input_filename, csv_reader=csv_reader, encoding=encoding, delimiter=delimiter
    )
    if has_header:
        header = next(reader)
    else:
    ...
    if parallel:
        ...
        with multiprocessing.Pool(processes=concurrency) as pool:
            map_args = [
                (filename, columns, numeric_column, encoding) for filename in filenames
            ]
    ...
    with open(
        output_filename or input_filename, "w", newline="", encoding=encoding
    ) as output_fp:
...
def main():
    ...
    parser.add_option(
        "-d", "--delimiter", default=",", help='set CSV delimiter (default ",")'
    )
    parser.add_option(
        "-e",
        "--encoding",
    ...
    if not input_files:
        ...
        args.columns = [
            int(column) if column.isdigit() else column for column in args.columns
        ]
        csvsort(
            input_files[0],
